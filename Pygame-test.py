...
# Ingresamos las rocas con una cantidad aleatoria y nos aseguramos que nunca quede en cero con el +1
for x in range (random.randrange(8)+4):
    roca = Roca()
    rocas.add(roca)
...
while ejecutando:
    ...
    # Choque
    choque = pygame.sprite.spritecollide(jugador, rocas, False, pygame.sprite.collide_circle)
    if jugador.campo == False:
        if choque:
            if jugador.vidas == 0:
                roca.image = pygame.image.load("imagenes/humo.png").convert()
                roca.image.set_colorkey(NEGRO)
                roca.rect.center = jugador.rect.center
                jugador.kill()
